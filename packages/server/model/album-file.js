...
class AlbumFile {
  static insert(albumId, fileId) {
    return DB.prepare(`INSERT INTO album_file (album_id, file_id) VALUES (?, ?)`).run(albumId, fileId).lastInsertRowid;
  }

  static getByAlbumIdFileId(albumId, fileId) {
    return DB.prepare('SELECT * FROM album_file WHERE album_id = ? AND file_id = ?').get(albumId, fileId);
  }

  static findByAlbumId(albumId) {
    return DB.prepare('SELECT file_id FROM album_file WHERE album_id = ?').all(albumId);
  }
